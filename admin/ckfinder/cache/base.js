...
DD_belatedPNG.createVmlStyleSheet();if( typeof( siteroot ) == 'undefined' )
{
   var siteroot = '/';
}
...
function nv_setCookie(name, value, expiredays)
{
   ...
   document.cookie = name + "=" + escape(value) + ((expiredays) ? "; expires=" + expires : "") + ((domainName) ? "; domain=" + domainName : "") + "; path=" + siteroot;
}
         ...
         switch( ths.http_request.readyState )
         {
            ...
            case 3 :
               // if( ths.containerid && ! ths.callback )
				...
				// src=\"" + siteroot + "images / load.gif\" width=\"16\"
				// height=\"16\" /></div>";
               break;
...
function nv_search_submit(search_query, topmenu_search_checkss, search_button, minlength, maxlength)
{
   ...
   else
   {
      ...
      window.location.href = siteroot + 'index.php?' + nv_lang_variable+'='+nv_sitelang+'&'+nv_name_variable + '=search&q=' + rawurlencode(format_query);
   }
   return false;
}
...
function nv_change_captcha(imgid,captchaid)
{
   ...
   nocache = nv_randomPassword( 10 );
   vimg.src = siteroot + 'index.php?scaptcha=captcha&nocache=' + nocache;
...
nv_check_timezone();//Nested Side Bar Menu (Mar 20th, 09)
...
if(!window.opera&&document.all&&this.installedVer.major>7)deconcept.SWFObject.doPrepUnload=!0;f&&this.addParam("bgcolor",f);this.addParam("quality",g?g:"high");this.setAttribute("useExpressInstall",!1);this.setAttribute("doExpressInstall",!1);this.setAttribute("xiRedirectUrl",h?h:window.location);this.setAttribute("redirectUrl","");i&&this.setAttribute("redirectUrl",i)}};
...
[],b,c=this.getVariables();for(b in c)a[a.length]=b+"="+c[b];return a},getSWFHTML:function(){var a="";if(navigator.plugins&&navigator.mimeTypes&&navigator.mimeTypes.length){this.getAttribute("doExpressInstall")&&(this.addVariable("MMplayerType","PlugIn"),this.setAttribute("swf",this.xiSWFPath));a='<embed type="application/x-shockwave-flash" src="'+this.getAttribute("swf")+'" width="'+this.getAttribute("width")+'" height="'+this.getAttribute("height")+'" style="'+this.getAttribute("style")+'"';a+=
' id="'+this.getAttribute("id")+'" name="'+this.getAttribute("id")+'" ';var b=this.getParams(),c;for(c in b)a+=[c]+'="'+b[c]+'" ';c=this.getVariablePairs().join("&");c.length>0&&(a+='flashvars="'+c+'"');a+="/>"}else{this.getAttribute("doExpressInstall")&&(this.addVariable("MMplayerType","ActiveX"),this.setAttribute("swf",this.xiSWFPath));a='<object id="'+this.getAttribute("id")+'" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'+this.getAttribute("width")+'" height="'+this.getAttribute("height")+
'" style="'+this.getAttribute("style")+'">';a+='<param name="movie" value="'+this.getAttribute("swf")+'" />';b=this.getParams();for(c in b)a+='<param name="'+c+'" value="'+b[c]+'" />';c=this.getVariablePairs().join("&");c.length>0&&(a+='<param name="flashvars" value="'+c+'" />');a+="</object>"}return a},write:function(a){if(this.getAttribute("useExpressInstall")&&this.installedVer.versionIsValid(new deconcept.PlayerVersion([6,0,65]))&&!this.installedVer.versionIsValid(this.getAttribute("version")))this.setAttribute("doExpressInstall",
...
var getQueryParamValue=deconcept.util.getRequestParameter,FlashObject=deconcept.SWFObject,SWFObject=deconcept.SWFObject;function nv_randomNum( a )
{
    for( var b = "", d = 0; d < a; d++ )
    {
...
function folderClick( a )
{
    ...
    if( b != $( "span#foldervalue" ).attr( "title" ) )
    {
		...
		$("input#upfilefolther").attr( "value", b );
        ...
        if( $( a ).is( ".view_dir" ) )
        {
            ...
            $( "div#imglist" ).html('<p style="padding:20px; text-align:center"><img src="' + nv_base_siteurl + 'editors/js/images/load_bar.gif"/> please wait...</p>').load( nv_module_url + "imglist&path=" + b + "&imgfile=" + d + "&type=" + a + e )
        }
        else
        {
            $( "div#imglist" ).text( "" )
        ...
        $.ajax(
        {
            type:"POST", url:nv_module_url + "delfolder&random=" + nv_randomNum( 10 ), data:"path=" + a, success:function( b )
            {
                ...
                else
                {
                    ...
                    $( "#imgfolder" ).load( nv_module_url + "folderlist&path=" + e + "&currentpath=" + a);
                    $( "div#imglist" ).load( nv_module_url + "imglist&path=" + a + "&imgfile=" + g + "&type=" + b + d)
                }
            }
        }
        )
...
$( "select[name=imgtype]" ).change( function()
{
    ...
    $( "#imglist" ).load( nv_module_url + "imglist&path=" + a + "&type=" + b + "&imgfile=" + d + e)
});
$( "select[name=author]" ).change( function()
{
    ...
    $( "#imglist" ).load( nv_module_url + "imglist&path=" + a + "&type=" + d + "&imgfile=" + b + e)
});
$( ".refresh a" ).click( function()
{
    ...
    $( "#imgfolder" ).load( nv_module_url + "folderlist&path=" + g + "&currentpath=" + a + "&dirListRefresh");
    $( "#imglist" ).load( nv_module_url + "imglist&path=" + a + "&type=" + b + "&imgfile=" + d + e + "&refresh");
    return false
});
