...
$sql = "SELECT * from technique where id = $id ";
$kq = $link->query($sql);
$row = $kq->fetch_array();
...
<div class="form-group">
    ...
    <input style="margin-bottom: 20px;" class="form-control" type="file" name="image" class="ava" ></input>
    <div class="form-group">
    <label> Video:</label>
    <input value="<?php echo $row['video']?>" name="video" type="text"  placeholder="Nhập link nhúng video" class="form-control" >
    </div>    
    <br>
     <div class="form-group">
    <label> Keyword:</label>
...
$sql = "UPDATE technique SET title = '$title' ,category='$category',author='$author', content='$content', description='$description', ava='$ava', image='$image', video='$video', keyword='$keyword', comment='$comment' where id = $id " ;
$link -> query($sql);

echo '<script> location.href="tech.php";</script>';
